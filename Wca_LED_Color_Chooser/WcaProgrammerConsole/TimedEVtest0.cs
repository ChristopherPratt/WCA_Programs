...
namespace WcaDVConsole
{
    ...
    class TimedEVtest0 : Target, IInterfaceListener, ICommandListener
    {
        ...
        public void Run(string[] comport, string[,] dacport)
        {
            myResend = resetCommand; // initiating the delegate
            rerunIndex = 0;
            myRerun = new rerun[10];
            deviceNum = comport.Length; // so i don't have to find the number of devices we are testing every time i need to loop through all the devices           
            device = new DeviceData[deviceNum];
            string time = getUTCtime();
            int offSet = 0; // each device will need to be offset from each other to prevent bogging or subthreads
            m_target = new Target(myResend);
            for (int a = 0; a < deviceNum; a++)
            {
                device[a].timerLengths = new List<int> { 104400, 43200, 25200  }; //times for the schedule for the test in half seconds ( because count the timer increments in half seconds
                device[a].repeatCount = 0;
                device[a].finalRepeatTotal = 10;
                //device[a].timerLengths = new List<int> { 30, 60, 90 };
                //device[a].timerLengths = new List<int> { 60, 120, 180 };
                //device[a].timerLengths = new List<int> { 120, 150, 270};
                device[a].enableVoltVeri = false; device[a].stopCommands = false; device[a].stopCoilOnce = false; device[a].startCoilOnce = false;
                device[a].activeCoil = 2;
                device[a].coilPF = new string[3];
                device[a].myDaq = new Daq(new string[] { dacport[a, 0], dacport[a, 1], dacport[a, 2] });
                device[a].comport = comport[a];
                device[a].m_si = new WcaInterfaceLibrary.SerialInterface(device[a].comport, globals.baud, Parity.None, 8); // actually setting the serial object array for the devices
                device[a].m_si.RegisterListener(this); // i think this is important, so we will copy the way we made the object array for serial
                if (!device[a].m_si.Open()) Console.WriteLine(device[a].comport + " port could not be opened."); //check if the port actually opened
                readAppID(a);
                readESN_HW(a);
                ...
                device[a].timerCount = offSet; // initializing each devices offset         
        ...
        {                                                                               // which responded with either a execution timeout or a neg ack. we put each failed command
            ...
            Console.WriteLine("Resent " + name + " on " + comport + " Command: " + myRerun[rerunIndex].command.ToString() + " Data:[" + myData + "] " + getUTCtime());
        ...
        private void SetTimer() // creates a timer which calls a function every .5 seconds.
        {
            aTimer = new System.Timers.Timer(500);  // Create a timer with a 0.5 second interval.            
            aTimer.Elapsed += Check; // Check is the function we are using every time the timer elapses
        ...
        private void Check(Object source, ElapsedEventArgs e) // im not sure what the arguments here do, but they are necessary for the timing class
        {
            ...
            for (int a = 0; a < deviceNum; a++) // iterate through all devices schedules every 500ms to check if it is time for an event
            {
                ...
                if (device[a].timerCount > device[a].timerLengths[0] && device[a].timerCount <= device[a].timerLengths[1]) // first timer here has 14.5 hours of normal operation until now, then stops sending commands
                {

                    device[a].stopCommands = true;
                }
                else if (device[a].timerCount > device[a].timerLengths[1] && device[a].timerCount <= device[a].timerLengths[2]) // second timer here waits 6 hours and then turn on normal function again
                {
                    if (!device[a].startCoilOnce)
                    {
                        Console.WriteLine("Cycle" + (device[a].repeatCount) + " start again");
                        device[a].startCoilOnce = true;
                        device[a].stopCommands = false;
                        setCoil(a);
                        setLED(a);
                        device[a].enableVoltVeri = false;
                    }                    
                }
                else if (device[a].timerCount > device[a].timerLengths[2]) // third timer waits 3.5 hours  of normal operation and then shuts down the application
                {
                    device[a].repeatCount++;
                    if (device[a].repeatCount >= device[a].finalRepeatTotal)
                    {
                    ...
                    {
                        Console.WriteLine("Cycle" + (device[a].repeatCount) + " completed!");
                        device[a].timerCount = 0;
                        device[a].stopCoilOnce = false;
                        device[a].startCoilOnce = false;
                        device[a].enableVoltVeri = false;
                        device[a].stopCommands = false;
                    }
                }
                    ...
                    if (device[a].enableVoltVeri) // only check coil voltage if the coil has been set for 10 seconds.
                    {
                        ...
                        for (int b = 0; b < 3; b++)
                        {
                            ...
                            else
                            {
                                ...
                                if (globals.debug)
                                {
                                    Console.WriteLine(getUTCtime() + " device " + device[a].esn + " failed to connect to coil " + device[a].activeCoil);
                ...
                else if (device[a].stopCommands) 
                {
                    if (!device[a].stopCoilOnce)
                    {
                        stopCoils(a);
                        device[a].stopCoilOnce = true;
                    }                    
                }      
                ...
                device[a].timerCount++; // increment timer count for each device for scheduling   
            }
        }
        ...
        public void OnNotification(ICommand sender) // this function recieves a verified message from the device which we can then parse and log.
        {
            ...
            else if (sender.isReceive)
            {
                ...
                if (!string.IsNullOrEmpty(str))
                {
                    ...
                    {
                        Console.Write(sender.Name + " Count " + device[deviceIndex].count + " Coil " + device[deviceIndex].activeCoil);
                        ...
                        {                    //     |Sync Val| message length |Source/Dest/Code|Command|    Data   | Checksum |  //data can contain subcommand info
                                        ...
                                        case "08": //readadcv
                                            {
                                                ...
                                                Console.Write(" Read ADC Voltage \t");
                                                break;
                                            }
                                        case "11": //readtemp
                                            {
                                        ...
                                        case "0A": //read current coil ma RMS
                                            {
                                                ...
                                                Console.Write(" Read ADC Current \t");
                                                break;
                                            }
                                        case "19": //read active coil
                                            {
                                                ...
                                                {
                                                    Console.Write(" Coil " + device[deviceIndex].activeCoil + " Confirmed \t");
                                                }
                                                else
                                                {
                                                    Console.Write(" Coil NOT Confirmed \t");
                                                    setCoil(deviceIndex);
                                                }
                                                break;
                                            }
                            ...
                            case "06": // set Runtime Parameter
                                {
                                    ...
                                    {
                                        case "19":
                                            Console.Write(" Set Coil " + device[deviceIndex].activeCoil + " on \t");
                                            break;
                                    }
                                    break;
                                }
                            case "1E": // read application version
                                {
                                    ...
                                    Console.Write(" Read App Version \t");
                                    break;
                                }
                            case "03": // read esn
                                {
                            ...
                            case "0E": // Read LED color
                                {
                                    ...
                                    if (BitConverter.ToInt32(red, 0) == 65535 && device[deviceIndex].ledColor.Equals("red"))
                                    {
                                        ...
                                        Console.Write(" Color " + device[deviceIndex].ledColor + " Confirmed \t");
                                    }
                                    ...
                                    {
                                        Console.Write(" Color NOT Confirmed \t");
                                        setLED(deviceIndex);
                                    }
                                    break;
                                }
                            case "0C": // set led
                                {
                                    Console.Write(" Set Color " + device[deviceIndex].ledColor + " \t");
                                    break;
                                }
                        ...
                        Console.WriteLine(getUTCtime() + " [" + str + "]");
                    }
                }
