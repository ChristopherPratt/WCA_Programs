...
namespace WcaDVConsole
{
    ...
    class EVtestMode : Target, IInterfaceListener, ICommandListener
    {
        ...
        //    public int count, activeCoil, flush;
        //    public double[] coilV;
        //    public StreamWriter sw;
        //    public Daq myDaq;
        //    public bool sledACK, rledACK, scoilACK, rcoilACK, esnACK, appidACK, cadcACK, vadcACK, tempACK, enableVoltVeri; //bool vars that tell if an ACK has been recieved for each command.

        //    //public bool[] failMode;

        //}
        //private string[] getDAQ(int mydutnum) // get the analog input ports of the DAQ that we need to collect the coil volatages
        //{
        //    switch (mydutnum)
        //    {

        //        case 1: return new string[] { "0", "8", "1" };
        //        case 2: return new string[] { "9", "2", "10" };
        //        case 3: return new string[] { "3", "11", "4" };
        //        case 4: return new string[] { "12", "5", "13" };
        //        case 5: return new string[] { "6", "14", "7" };
        //        case 6: return new string[] { }; // daq doesn't have enough analog input ports for 6 DUTs
        //    }
        //    return new string[] { };
        //}
        ...
        public void Run(List<Commands.DeviceData> d, Commands c)
        {
            ...
            for (int a = 0; a < deviceNum; a++)
            {
                ...
                //if (!device[a].m_si.Open()) commands.writeLineToConsole(device[a].comport + " port could not be opened."); //check if the port actually opened
                readAppID(a);
                readESN_HW(a);
            ...
            SetTimer();
            while (!end) { Thread.Sleep(500); } // if the user types end then the program exits
            for (int b = 0; b < deviceNum; b++) { stopCoils(b); device[b].m_si.Close(); device[b].sw.Flush(); device[b].sw.Close();} // close serial port and close log file
            commands.writeLineToConsole("Done");
            Thread.Sleep(1000);
