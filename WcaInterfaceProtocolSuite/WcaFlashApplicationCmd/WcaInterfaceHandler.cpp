	...
	if(m_IsInitialized)
	{
		...
		if (res == WcaInterfaceCommandResult_PosAck)
		{
			if (sf_out != NULL)
			{
				...
				for (ULONG i = sf_out->rgsabound[0].lLbound ; i < sf_out->rgsabound[0].cElements && i<(ULONG)size; ++i)
				{
					*(*prdata+i) = *(bp + i);
					*written = *written +1;
				}
...
std::string WcaInterfaceHandler::GetVersion(void)
{
	std::string logs = "";
	BSTR* pRetVal = NULL;
	...
	if (m_IsInitialized)
	{
		m_pWcaInterface->GetVersion(pRetVal);

		if (pRetVal != NULL)
		{
			logs = _bstr_t(*pRetVal);
		}
	}
	return logs;
}
